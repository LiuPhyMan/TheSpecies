...
class AbsSpecie(object):
  ...
  def __init__(self, *, spc_str: str) -> None:
    ...
    self.Zc = _spc["Zc"]
    self.Hf = _spc["Hf"]
    self.ionE = float(_spc["ionE"]) if not _spc["ionE"].strip() == "" else 0
    self.polar = float(_spc["polar"]) if not _spc["polar"].strip() == "" else 0
...
class SpecieWithLevel(AbsSpecie):
  ...
  def rdcd_mu0(self, *, T_K):
    return 3.66487052 - 1.5*log(self.relM) - 2.5*log(T_K) - log(self.qint(
      T_K)) \
      + self.Hf*eV2K/T_K
...
class SpecieWithQint(AbsSpecie):
  ...
  def rdcd_mu0(self, *, T_K: float):
    return 3.66487052 - 1.5*log(self.relM) - 2.5*log(T_K) - log(
      self.qint(T_K)) + \
      self.Hf*eV2K/T_K
  ...
  #     return 2.5 + 2 * T_K * self.dlnqintdT(T_K) + T_K**2 *
  #     self.dlnqintdT_2(T_K)
...
for _str in spec_info[:10]:  # top 10 lines.
  _key, _type, _path = [_.strip() for _ in _str.split(",")]
  if _type == "level":
    spc_dict[_key] = SpecieWithLevel(spc_str=_key)
    spc_dict[_key].set_gE(gE_file=parentPath/'data'/_path)
  elif _type == "qint":
    spc_dict[_key] = SpecieWithQint(spc_str=_key)
    spc_dict[_key].set_qint(qint_file=parentPath/'data'/_path)
  else:
    raise Exception(f"The type {_type} is error.")

# spc_dict["Ar"].set_emiss_lines(lineFile=f"{__path__[0]}/data/line/Ar/Ar
# I.txt")
# spc_dict["Ar_1p"].set_emiss_lines(lineFile=f"{__path__[0]}/data/line/Ar/Ar
# II.txt")
# spc_dict["Ar_2p"].set_emiss_lines(lineFile=f"{__path__[0]}/data/line/Ar/Ar
# III.txt")
# spc_dict["Ar_3p"].set_emiss_lines(lineFile=f"{__path__[0]}/data/line/Ar/Ar
# IV.txt")
# spc_dict["Ar_4p"].set_emiss_lines(lineFile=f"{__path__[0]}/data/line/Ar/Ar
# V.txt")
# spc_dict["Xe"].set_emiss_lines(lineFile=f"{__path__[0]}/data/line/Xe/Xe
# I.txt")
# spc_dict["Xe_1p"].set_emiss_lines(lineFile=f"{__path__[0]}/data/line/Xe/Xe
# II.txt")
# spc_dict["Xe_2p"].set_emiss_lines(lineFile=f"{__path__[0]}/data/line/Xe/Xe
# III.txt")
# spc_dict["Xe_3p"].set_emiss_lines(lineFile=f"{__path__[0]}/data/line/Xe/Xe
# IV.txt")
# spc_dict["Xe_4p"].set_emiss_lines(lineFile=f"{__path__[0]}/data/line/Xe/Xe
# V.txt")
# spc_dict = dict()
